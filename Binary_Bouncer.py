...
from LSTM_model import loadModel,executeModel_tune, stringHandler, evaluate, categoryFromOutput, randomTrainingExample, timeSince,lineToTensor
...
@click.option(
    '--enable', '-e', is_flag=True,help="set this flag to enable automatic validation testing of trained LSTM model"
)
@click.option(
    '--seed', '-s', is_flag=True,help="set this flag to seed pyTorch and random computations (if you need an exact reproducibility) normally unnecessary"
)
@click.argument(
    'incoming_binary'
)
...
def ml_validation(lstm):
    ...
    for i in range(n_validators):
        ...
        print('%d %d%% (%s) %s / %s %s' % (i, i / n_validators * 100, timeSince(start), line, guess, answer))
...
def bounce(incoming_binary,lstm, binaryType):
    #pdb.set_trace()
    subprocess.Popen("strings " + str(incoming_binary) + " > ./data/incoming_data.txt" ,shell=True)
    ...
    if incoming_data.is_file(): 
        ...
        if(file_type == "elf"):
            ...
            subprocess.Popen("readelf -p .strtab " + str(incoming_binary) + " >> " + preset['import_bucket'] ,shell=True)
            # grab NEEDED dynamic section symbols
            subprocess.Popen("objdump -p " + str(incoming_binary) + " | grep NEEDED  >> " + preset['tempData'] , shell=True)
            time.sleep(5)
            ...
            for line in f:
                if (re.search("Section '.strtab' was not dumped because it does not exist!", line)):
            ...
            for exp in pe.DIRECTORY_ENTRY_EXPORT.symbols:
                if(exp.name is not None):
                    ...
                    print(hex(pe.OPTIONAL_HEADER.ImageBase + exp.address), str(exportSymbol))
                    f.write('\n' + str(exportSymbol))
