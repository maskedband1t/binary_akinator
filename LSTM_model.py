...
def randomTrainingExample():
    ...
    category_tensor = torch.tensor(
        [preset["all_categories"].index(category)], dtype=torch.long
    )
...
class LSTMClassifier(nn.Module):
    def __init__(self, n_letters, hidden_sz, n_layers, n_categories=2) -> None:
        ...
        self.network = nn.LSTM(
            n_letters, hidden_sz, n_layers, batch_first=True
        )  # batch_sz X 1 X hidden_sz
        self.linear = nn.Linear(
            hidden_sz, n_categories
        )  # hidden_sz X n_categories, gives us unnormalized probabilit
...
def executeModel_tune():
    ...
    for iter in range(1, preset["n_iters"] + 1):
        ...
        if iter % preset["print_every"] == 0:
            ...
            if guess == category:
                correct = "✓"
                if iter > (preset["n_iters"] + 1 - record_last_x) and iter < (
                    preset["n_iters"] + 1
                ):
